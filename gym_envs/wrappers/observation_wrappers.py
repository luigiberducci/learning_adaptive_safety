...
from gym_envs.multi_agent_env.common.track import (
    ...
    extract_forward_raceline, extract_forward_boundary, Raceline,
)
...
class VehicleTrackObservationWrapper(gym.ObservationWrapper):
    def __init__(
        ...
        self.vehicle_features = vehicle_features or ["pose", "velocity", "frenet_coords"]
...
class VectorTrackObservationWrapper(VehicleTrackObservationWrapper):

    def track_feature_space_factory(self, feature_name: str) -> gym.spaces.Box:
        if feature_name == "left_boundary":
            return gym.spaces.Box(
                low=-np.inf, high=np.inf, shape=(self.n_raceline_points, 2), dtype=np.float32
            )
            ...
            return gym.spaces.Box(
                low=-np.inf, high=np.inf, shape=(self.n_raceline_points, 3), dtype=np.float32
            )
        else:
            return super().track_feature_space_factory(feature_name)
        ...
        for i, nid in enumerate(closest_ids):
            ...
            for feature in self.vehicle_features:
                ...
                if feature == "pose":
                    ...
                    rel_pos = traj_global2local(ego_pose=ego_feature, traj=other_feature[:2][None])
                    rel_head = other_feature[2] - ego_feature[2]
                    rel_feature = np.array([rel_pos[0][0], rel_pos[0][1], rel_head])
                else:
                    rel_feature = other_feature - ego_feature
                obs[new_agent_id][feature] = rel_feature
        # zero padding for missing vehicles
        for i in range(self.n_vehicles - len(closest_ids)):
            new_agent_id = f"vehicle_nearby{len(closest_ids) + i}"
            obs[new_agent_id] = {}
    ...
    def observe_context_feature(
        ...
        if feature_name == "vec_raceline":
            ...
            all_boundary = self.left_boundary_xy if feature_name == "left_boundary" else self.right_boundary_xy
            ...
            return super().observe_context_feature(feature_name, obs)
...
if __name__ == "__main__":
    ...
    env = VectorTrackObservationWrapper(
        env,
        ...
        if debug and t % 50 == 0:
            ...
            for vec_polyline in [vec_raceline, vec_lboundary, vec_rboundary]:
                ...
                for i in range(vec_polyline.shape[0] - 1):
                    ...
                    ax1.arrow(starting_pt[0], starting_pt[1], vec_polyline[i + 1, 0], vec_polyline[i + 1, 1],
                              head_width=0.02)
