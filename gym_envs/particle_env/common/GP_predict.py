...
class GP:
    ...
    def __init__(
        self, X, Y, kernel="SE", omega=None, l=None, sigma=None, noise=None, horizon=20
    ):
        self.X = X
        self.Y = Y
        self.X_s = X
    ...
    def predict(self, Xnew):
        ...
        Sigma = (
            self.evaluate_kernel(Xnew, Xnew)
            + self.noise
            - np.matmul(np.transpose(np.matmul(K_inv, k_star)), k_star)
        )
