...
class Car:
    ...
    def get_dynamics_human(self, x, t=1):
        ...
        gp = np.array([[0.0, 0.0], [0.0, 0.0]])
        fv = vel
        gv = np.array([[self.dt, 0.0], [0.0, self.dt]])
        return fp, gp, fv, gv
    ...
    def fh_err(self, x):
        ...
        v_nom = vel
        v = v_nom
        v[0] = max(-self.max_velocity, min(v[0], self.max_velocity))
        v[1] = max(-self.max_velocity, min(v[1], self.max_velocity))
        return p, v
